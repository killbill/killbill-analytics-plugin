...
from analytics_account_tags b
     ...
     left outer join ( select id,name from tag_definitions
                       union select '00000000-0000-0000-0000-000000000001','AUTO_PAY_OFF'
                       union select '00000000-0000-0000-0000-000000000002','AUTO_INVOICING_OFF'
                       union select '00000000-0000-0000-0000-000000000006','TEST'
                       union select '00000000-0000-0000-0000-000000000003','OVERDUE_ENFORCEMENT_OFF'
                       union select '00000000-0000-0000-0000-000000000005','MANUAL_PAY'
                       union select '00000000-0000-0000-0000-000000000007','PARTNER' ) td on t.tag_definition_id = td.id
where coalesce(b.tag_record_id, 'NULL') != coalesce(t.record_id,'NULL')
      or coalesce(b.name, 'NULL') != coalesce(td.name,'NULL')
      or coalesce(b.created_date, 'NULL') != coalesce(t.created_date,'NULL')
      or coalesce(b.account_record_id, 'NULL') != coalesce(t.account_record_id,'NULL')
...
-- F3a
select *
  from ( select invoice_id, invoice_amount_charged, sum(coalesce(amount,0)) bii_sum from analytics_invoice_items group by invoice_id,invoice_amount_charged, invoice_original_amount_charged ) bii_sum
       left outer join  ( select invoice_id, sum(coalesce(amount,0)) bia_sum from analytics_invoice_adjustments group by invoice_id ) bia_sum using (invoice_id)
       left outer join ( select invoice_id, sum(coalesce(amount,0)) biia_sum from analytics_invoice_item_adjustments group by invoice_id ) biia_sum using (invoice_id)
where bii_sum + coalesce(bia_sum,0) + coalesce(biia_sum,0) != bii_sum.invoice_amount_charged
;

-- F3b
select bin.invoice_id, bin.original_amount_charged, sum(bii.amount)
 from analytics_invoice_items bii
...
-- F3c
select *
  from ( select invoice_id, invoice_amount_credited, sum(coalesce(amount,0)) biic_sum from analytics_invoice_credits biic group by invoice_id,invoice_amount_credited ) biic_sum 
...
select *, 
  ...
  from ( select invoice_id, invoice_amount_paid, sum(coalesce(amount,0)) bip_sum from analytics_payments bip group by invoice_id, invoice_amount_paid ) bip_sum 
where bip_sum != bip_sum.invoice_amount_paid
;

-- F3e
select *
  from ( select invoice_id, balance, amount_charged, amount_credited, amount_paid, amount_refunded, original_amount_charged from bin ) bin
                left outer join ( select invoice_id, sum(coalesce(amount,0)) bipc_sum from analytics_payments bipc group by invoice_id ) bipc_sum using (invoice_id)
                left outer join ( select invoice_id, sum(coalesce(amount,0)) bipr_sum from analytics_refunds bipr group by invoice_id ) bipr_sum using (invoice_id)
...
-- G6aii
-- Litle
-- NOT WORKING or lots problems
select *
from bip
     left outer join litle_transactions lt on bip.payment_id = lt.kb_payment_id
     left outer join litle_responses lr on bip.payment_id = lr.kb_payment_id
where ( 1=0
--  or coalesce(lt.created_at, 'NULL') != coalesce(bip.plugin_created_date,'NULL')
  --    or coalesce(lt.created_at, 'NULL') != coalesce(bip.plugin_effective_date,'NULL')
      -- or coalesce(pp.z_status, 'NULL') != coalesce(bip.plugin_status,'NULL')
      -- or coalesce(lr.params_litleonelineresponse_saleresponse_message, 'NULL') != coalesce(bip.plugin_gateway_error,'NULL')
--      or coalesce(lr.message, 'NULL') != coalesce(bip.plugin_gateway_error,'NULL')
      or coalesce(lr.params_litleonelineresponse_saleresponse_response, 'NULL') != coalesce(bip.plugin_gateway_error_code,'NULL')
--      or coalesce(lr.params_litleonelineresponse_saleresponse_litle_txn_id, 'NULL') != coalesce(bip.plugin_first_reference_id,'NULL')
 --     or coalesce(lr.id, 'NULL') != coalesce(bip.plugin_second_reference_id,'NULL')
 ) 
      and plugin_name = 'killbill-litle'
...
select *
from bip
     left outer join _zuora_payments pp on bip.payment_id = pp.kb_p_id
where ( coalesce(pp.z_created_date, 'NULL') != coalesce(bip.plugin_created_date,'NULL')
      or coalesce(pp.z_effective_date, 'NULL') != coalesce(bip.plugin_effective_date,'NULL')
      or coalesce(pp.z_status, 'NULL') != coalesce(bip.plugin_status,'NULL')
      or coalesce(pp.z_gateway_error, 'NULL') != coalesce(bip.plugin_gateway_error,'NULL')

...
SELECT
    ...
    ,sum(case when apc.created_date >= FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - 15*60 - (UNIX_TIMESTAMP(NOW()) - 15*60)%(15*60))
    ...
    ,sum(case when apc.created_date >= FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - 15*60 - (UNIX_TIMESTAMP(NOW()) - 15*60)%(15*60))
				   and cast(FROM_UNIXTIME(UNIX_TIMESTAMP(apc.created_date)- UNIX_TIMESTAMP(apc.created_date)%(15*60)) as time) = cast(FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - 15*60 - (UNIX_TIMESTAMP(NOW()) - 15*60)%(15*60)) as time)
                   then 1
              else 0 end) as current_transaction_count
    ,sum(case when apc.created_date < FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - 15*60 - (UNIX_TIMESTAMP(NOW()) - 15*60)%(15*60))
                   and cast(FROM_UNIXTIME(UNIX_TIMESTAMP(apc.created_date)- UNIX_TIMESTAMP(apc.created_date)%(15*60)) as time) = cast(FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - 15*60 - (UNIX_TIMESTAMP(NOW()) - 15*60)%(15*60)) as time)
				   and apc.payment_transaction_status='SUCCESS'
                   then 1
    ...
    ,sum(case when apc.created_date < FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - 15*60 - (UNIX_TIMESTAMP(NOW()) - 15*60)%(15*60))
                   and cast(FROM_UNIXTIME(UNIX_TIMESTAMP(apc.created_date)- UNIX_TIMESTAMP(apc.created_date)%(15*60)) as time) = cast(FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - 15*60 - (UNIX_TIMESTAMP(NOW()) - 15*60)%(15*60)) as time)
                   then 1
              else 0 end) as historical_transaction_count
FROM
...
SELECT
    ...
    ,sum(case when app.created_date >= FROM_UNIXTIME(UNIX_TIMESTAMP(NOW()) - 15*60 - (UNIX_TIMESTAMP(NOW()) - 15*60)%(15*60))
